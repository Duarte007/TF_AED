...
namespace BancoSMEM
{
	class ArvoreOperacoes
	{
		...
		private Operacao adicionar(Operacao raizArvore, Operacao operacaoNovo)
		{
			///  Se  a  raiz  da  árvore  ou  sub-­‐árvore  for  null,  a  árvore  está  vazia e então um novo aluno é inserido.
			if (raizArvore == null) raizArvore = operacaoNovo;
			else
			{
				...
				if (Convert.ToInt64(raizArvore.numConta) > Convert.ToInt64(operacaoNovo.numConta)) raizArvore.esquerda = adicionar(raizArvore.esquerda, operacaoNovo);
				else
				{
					...
					if (Convert.ToInt64(raizArvore.numConta) < Convert.ToInt64(operacaoNovo.numConta)) raizArvore.direita = adicionar(raizArvore.direita, operacaoNovo);
					else
		...
		public void inserir(Operacao operacaoNovo)
		{
			...
			this.raiz = adicionar(this.raiz, operacaoNovo);
		}
		...
		private Operacao antecessor(Operacao operacaoRetirar, Operacao raizArvore)
		{
			...
			if (raizArvore.direita != null)
			{
				...
				raizArvore.direita = antecessor(operacaoRetirar, raizArvore.direita);
				return raizArvore;
			}
		...
		private Operacao retirar(Operacao raizArvore, int numConta)
		{
			...
			if (raizArvore == null)
			{
				MessageBox.Show("A conta, cujo numero é " + numConta + ", não foi encontrado.");
				return raizArvore;
			}
			else
			{
				...
				if (raizArvore.numConta.Equals(numConta))
				{
				...
				else
				{
					...
					if (Convert.ToInt64(raizArvore.numConta) > numConta)
						raizArvore.esquerda = retirar(raizArvore.esquerda, numConta);
					else
						...
						raizArvore.direita = retirar(raizArvore.direita, numConta);
		...
		public void remover(int operacaoRemover)
		{
			...
			this.raiz = retirar(this.raiz, operacaoRemover);
		}

		private Operacao pesquisar(Operacao raizArvore, long numConta)
		{
			if (raizArvore == null)
			{
			...
			else
			{
				...
				{
					if (Convert.ToInt64(raizArvore.numConta) > numConta)
						raizArvore.esquerda = pesquisar(raizArvore.esquerda, numConta);
					else
						raizArvore.direita = pesquisar(raizArvore.direita, numConta);
					return raizArvore;
				}
			}
		}

		public Operacao encontrar(long numConta)
		{
			this.raiz = pesquisar(this.raiz, numConta);
			return this.raiz;
		}
	}
}
